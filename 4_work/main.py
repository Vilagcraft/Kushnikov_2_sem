...
def du_dt(t, u, f):
    ...
    dx2_dt = (
        ( f['Z1'](t) + f['Z2'](t)  + f['Z4'](t) )
        * f['f4'](x4_t) * f['f5'](x6_t) * f['f6'](x4_t)
        -
    ...
    return [
        clamp(dx1_dt), clamp(dx2_dt), clamp(dx3_dt), clamp(dx4_dt),
        clamp(dx5_dt), clamp(dx6_dt), clamp(dx7_dt), clamp(dx8_dt),
        clamp(dx9_dt), clamp(dx10_dt), clamp(dx11_dt), clamp(dx12_dt),
        clamp(dx13_dt), clamp(dx14_dt), clamp(dx15_dt),
        clamp(dx16_dt), clamp(dx17_dt), clamp(dx18_dt)
    ]
...
async def count(data  = Body()):
    try:
        ...
        print("Keys in f:", f.keys())
        usolution = solve_ivp(
            du_dt,
            (0, 1),
            [t0[f"X{i}"] for i in range(1, 19)],
            args=(f,),
            dense_output=True,
            rtol=1e-6,
            atol=1e-9,
        ...
        if (not usolution.success):
            print("Solver failed:", usolution.message)
            raise HTTPException(status_code=500)

        t = [ i * 0.05 for i in range(21)]
        sol = usolution.sol(t)
        y = [
            [float(v) if v == v else 0 for v in row]  # защита от NaN
            for row in sol
        ]
